...
generate_test_data <- function(ntest, p, method = c("halton", "grid")){
...
simulation_settings_0 <- function(seed){
  ## integer -> tibble
  ## simulation setting as in gbex paper for step function
...
simulation_settings_00 <- function(seed){
  ## integer -> tibble
  ## simulation setting as in gbex paper for gaussian function
...
simulation_settings_5 <- function(seed){
  ## integer -> tibble
  ## impact of ntest
...
wrapper_sim <- function(i, sims_args, type = c("ise", "plot"),
  ...
  if (!inspect_erf){
    ...
    return(list(res = res,
                erf_object = tibble(X1 = X_test[, 1], X2 = X_test[, 2]) %>%
                  bind_cols(erf_object)))
  }
}
